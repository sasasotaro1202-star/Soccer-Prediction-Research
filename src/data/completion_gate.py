...
from src.features.soccer_features import build_match_features, add_target
...
def _pit_preflight(root: Path) -> dict:
    ...
    # Chronological OOS requires the matured match label in the replay handoff.
    # Keep PIT status/provenance generated by build_match_features intact, and add
    # the outcome label as a separate deterministic merge from the same historical
    # fixture identity. The downstream controller never uses current-block labels
    # for model fitting or routing.
    features = add_target(features, history)
