...
def coverage_matrix(history,field_rows,acquisition=None,config=AuditConfig()):
    ...
    for comp in TARGET_COMPETITIONS:
        ...
        if comp not in observed: rows.append({"competition":comp,"competition_name":COMPETITION_NAMES[comp],"season":pd.NA,"source":"current_observed_adapter","field":pd.NA,"status":"UNAVAILABLE","fixture_count":0,"reason":"No observed rows from current adapter; no data elsewhere is not claimed."})
...
def source_reconciliation(history):
    ...
    for key,g in x.groupby("canonical_key",sort=False):
        sources=sorted(set(g["source_name"].astype(str))); duplicate=len(g)>len(sources); outcome=[c for c in ("home_goals","away_goals","result") if c in g.columns]; sig=set(tuple(None if pd.isna(r[c]) else str(r[c]) for c in outcome) for _,r in g.iterrows()); collision=len(sources)>1 and len(sig)>1
        rows.append({"canonical_key":key,"source_count":len(sources),"sources":"|".join(sources),"duplicate_source_identity":bool(duplicate),"row_count":int(len(g)),"collision":bool(collision),"collision_reason":"Conflicting outcome across sources" if collision else ""})
