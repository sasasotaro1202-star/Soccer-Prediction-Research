...
def _normalise_scope_season(comp, raw):
    value = str(raw).strip()
    ...
    if value.isdigit() and len(value) == 4:
        return _season_key(comp, int(value))
    return value


def _scope_accounting(acquisition, config=AuditConfig()):
    expected = {
        (comp, _season_key(comp, year))
    ...
    if acquisition is not None and not acquisition.empty:
        for _, row in acquisition.iterrows():
            ...
            if comp not in TARGET_COMPETITIONS:
                continue
            season = _normalise_scope_season(comp, row.get("season", ""))
            cell = (comp, season)
            if cell in expected:
                accounted.add(cell)
    return len(accounted), len(expected - accounted)
