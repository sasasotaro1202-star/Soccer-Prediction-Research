...
def run_audit(out_dir="artifacts",config=AuditConfig()):
    ...
    fixtures=fixture_audit(history); fields=field_audit(history,config); coverage=coverage_matrix(history,fields,acquisition,config); reconciliation=source_reconciliation(history); pit=pit_audit(fields); plans=pd.DataFrame([{"competition":p.competition,"canonical_candidates":"|".join(p.canonical_candidates),"discovery_only":"|".join(p.discovery_only),"pit_status":p.pit_status,"notes":p.notes} for p in source_plans()]); fixtures.to_csv(out/"fixture_audit.csv",index=False); fields.to_csv(out/"field_audit.csv",index=False); coverage.to_csv(out/"coverage_matrix.csv",index=False); acquisition.to_csv(out/"acquisition_coverage.csv",index=False); reconciliation.to_csv(out/"source_reconciliation.csv",index=False); pit.to_csv(out/"pit_audit.csv",index=False); plans.to_csv(out/"competition_source_plan.csv",index=False); observed=sorted(set(history["competition"].astype(str))) if not history.empty else []; target_cells=len(TARGET_COMPETITIONS)*(config.end_year-config.start_year+1); expected_cells={(comp,_season_key(comp,year)) for comp in TARGET_COMPETITIONS for year in range(config.start_year,config.end_year+1)}; accounted_cells=set();
    for _,row in acquisition.iterrows():
        comp=str(row.get("competition","")).strip()
        if comp not in TARGET_COMPETITIONS: continue
        raw=str(row.get("season","")).strip()
        if "/" in raw:
            left,right=(part.strip() for part in raw.split("/",1)); raw=f"{left}/{right[-2:]}" if left.isdigit() and right.isdigit() and len(left)==4 and len(right)==4 else raw
        elif raw.isdigit() and len(raw)==4:
            raw=_season_key(comp,int(raw))
        cell=(comp,raw)
        if cell in expected_cells: accounted_cells.add(cell)
    accounted_cells=len(accounted_cells); missing_audit_cells=len(expected_cells-set((c,s) for c,s in [(r.get("competition"),str(r.get("season"))) for _,r in acquisition.iterrows()] if c in TARGET_COMPETITIONS)); summary={"target_competitions":list(TARGET_COMPETITIONS),"target_competition_count":len(TARGET_COMPETITIONS),"requested_season_start":config.start_year,"requested_season_end":config.end_year,"requested_competition_season_cells":target_cells,"historically_not_applicable_cells":sum(1 for comp in TARGET_COMPETITIONS for y in range(config.start_year,config.end_year+1) if not _is_applicable(comp, y)),"observed_competitions":observed,"observed_competition_count":len(observed),"unobserved_competitions":[c for c in TARGET_COMPETITIONS if c not in observed],"fixture_count":int(len(fixtures)),"field_observation_count":int(len(fields)),"pit_status_counts":{str(k):int(v) for k,v in (fields["pit_status"].value_counts().to_dict() if not fields.empty else {}).items()},"duplicate_source_rows":int(reconciliation["duplicate_source_identity"].sum()) if not reconciliation.empty else 0,"collision_rows":int(reconciliation["collision"].sum()) if not reconciliation.empty else 0,"team_mapping_invalid_rows":int((fixtures["team_mapping_status"]=="INVALID").sum()) if not fixtures.empty else 0,"no_missing_to_zero":True,"search_engines_are_not_dataset_sources":True,"audit_scope":"fixture -> competition/season/source/field -> duplicate/collision -> team identity -> PIT publication time","coverage_claim_policy":"observed-only; provider advertisements and search results never become AVAILABLE","audit_execution_ok":True,"coverage_scope_complete":bool(accounted_cells == target_cells and missing_audit_cells == 0),"pit_publication_time_gate":bool(not fields.empty and not (fields["pit_status"]=="PIT_UNKNOWN").any()),"full_gate_passed":False}; (out/"audit_summary.json").write_text(json.dumps(summary,indent=2,ensure_ascii=False,default=str),encoding="utf-8"); return summary
