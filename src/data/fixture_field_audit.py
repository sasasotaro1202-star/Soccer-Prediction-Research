...
def coverage_matrix(history,field_rows,acquisition=None,config=AuditConfig()):
    ...
    if acquisition is not None and not acquisition.empty:
        for _,r in acquisition.iterrows():
            ...
            season=raw if "/" in raw or not raw.isdigit() else (raw if comp in {"J1","J2","J3","FRI"} else f"{raw}/{str(int(raw)+1)[-2:]}")
...
def run_audit(out_dir="artifacts",config=AuditConfig()):
    out=Path(out_dir); out.mkdir(parents=True,exist_ok=True); history,acquisition=load_available_history(start_year=config.start_year,end_year=config.end_year); fixtures=fixture_audit(history); fields=field_audit(history,config); coverage=coverage_matrix(history,fields,acquisition,config); reconciliation=source_reconciliation(history); pit=pit_audit(fields); plans=pd.DataFrame([{"competition":p.competition,"canonical_candidates":"|".join(p.canonical_candidates),"discovery_only":"|".join(p.discovery_only),"pit_status":p.pit_status,"notes":p.notes} for p in source_plans()]); fixtures.to_csv(out/"fixture_audit.csv",index=False); fields.to_csv(out/"field_audit.csv",index=False); coverage.to_csv(out/"coverage_matrix.csv",index=False); acquisition.to_csv(out/"acquisition_coverage.csv",index=False); reconciliation.to_csv(out/"source_reconciliation.csv",index=False); pit.to_csv(out/"pit_audit.csv",index=False); plans.to_csv(out/"competition_source_plan.csv",index=False); observed=sorted(set(history["competition"].astype(str))) if not history.empty else []; target_cells=14*(config.end_year-config.start_year+1); summary={"target_competitions":list(TARGET_COMPETITIONS),"target_competition_count":14,"requested_season_start":config.start_year,"requested_season_end":config.end_year,"requested_competition_season_cells":target_cells,"historically_not_applicable_cells":4,"observed_competitions":observed,"observed_competition_count":len(observed),"unobserved_competitions":[c for c in TARGET_COMPETITIONS if c not in observed],"fixture_count":int(len(fixtures)),"field_observation_count":int(len(fields)),"pit_status_counts":{str(k):int(v) for k,v in (fields["pit_status"].value_counts().to_dict() if not fields.empty else {}).items()},"duplicate_source_rows":int(reconciliation["duplicate_source_identity"].sum()) if not reconciliation.empty else 0,"collision_rows":int(reconciliation["collision"].sum()) if not reconciliation.empty else 0,"team_mapping_invalid_rows":int((fixtures["team_mapping_status"]=="INVALID").sum()) if not fixtures.empty else 0,"no_missing_to_zero":True,"search_engines_are_not_dataset_sources":True,"audit_scope":"fixture -> competition/season/source/field -> duplicate/collision -> team identity -> PIT publication time","coverage_claim_policy":"observed-only; provider advertisements and search results never become AVAILABLE","audit_execution_ok":True,"coverage_scope_complete":bool(len(coverage)>=target_cells),"pit_publication_time_gate":bool(not fields.empty and not (fields["pit_status"]=="PIT_UNKNOWN").any()),"full_gate_passed":False}; (out/"audit_summary.json").write_text(json.dumps(summary,indent=2,ensure_ascii=False,default=str),encoding="utf-8"); return summary
