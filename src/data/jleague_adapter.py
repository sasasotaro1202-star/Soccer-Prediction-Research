...
The source is one machine-readable file (JPN.csv). Competition is taken from
its League column when present; rows are never guessed into J1/J2/J3 when the
source does not identify the competition.
"""
...
def load_jleague_history(start_year: int = 2010, end_year: int = 2025, cache_dir: str = "data/raw/jleague") -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    out["season_start"] = out["kickoff_utc"].dt.year.where(out["kickoff_utc"].dt.month >= 2, out["kickoff_utc"].dt.year - 1)
    # J.League's calendar-season competitions use the calendar year. Keep the
    # audit season key explicit rather than pretending they are European seasons.
    ...
    for comp in ("J1", "J2", "J3"):
        g = out[out["competition"] == comp]
        for season, sg in g.groupby("season"):
            coverage_rows.append({"competition": comp, "season": season, "status": "AVAILABLE", "rows": len(sg), "source": "Football-Data.co.uk:JPN.csv"})
