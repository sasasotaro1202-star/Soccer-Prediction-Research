...
# Ordered oldest-to-newest immutable snapshots. A row uses the earliest
# snapshot that contains the exact result and whose commit observation time
# remains valid under the explicit result-availability lower bound.
SNAPSHOTS = {
    "EPL": (
        {
        ...
        {
            "path": "data-raw/england.csv",
            "commit_sha": "aefaea3d194afb66e19a2695d15d5e5a24cfd1f5",
            "blob_sha": "ebd5bda21eb10ee8289cc0b792eadf97c714b8dd",
            "observed_at_utc": "2020-10-17T21:07:43+00:00",
        },
        {
            "path": "data-raw/england.csv",
    ...
    "LL": (
        {
            ...
            "commit_sha": "a14c9e4d771d4b13b62f1de6aaaa263ceab7a55e",
            "blob_sha": "1059d7cde1a1ff5050f2679351c9ad29d50534fa",
            "observed_at_utc": "2017-01-12T21:56:11+00:00",
        },
        {
            "path": "data-raw/spain.csv",
    ...
    "SA": (
        {
            ...
            "commit_sha": "f79c690b69f5cdd4217d3df7597b543823eed1ee",
            "blob_sha": "c1f03db8d2a4f2e2440243c3d6079ac87f5c714e",
            "observed_at_utc": "2019-08-23T14:28:17+00:00",
        },
        {
            "path": "data-raw/italy.csv",
            "commit_sha": "58edbe38504eaefefac0f5fcfe2ee38841106bba",
            "blob_sha": "db85a01dd5ff578b0a9014836fd71fde8f05105f",
            "observed_at_utc": "2020-10-24T16:57:03+00:00",
        },
        {
            "path": "data-raw/italy.csv",
    ...
    "ERE": (
        {
            ...
            "observed_at_utc": "2020-10-24T04:11:43+00:00",
        },
        {
            "path": "data-raw/holland.csv",
...
def apply_snapshot_pit(history: pd.DataFrame, *, cache_dir: str = "data/raw/pit_evidence", timeout: int = 45) -> pd.DataFrame:
    ...
    for competition, snapshots in SNAPSHOTS.items():
        ...
        prepared: list[tuple[datetime, dict[str, str], set[tuple]]] = []
        ...
        for snapshot in snapshots:
            ...
            try:
                ...
                prepared.append((observed_at, snapshot, keys))
            except Exception as exc:
                snapshot_errors.append(
                    f"{snapshot['commit_sha']}:{type(exc).__name__}:{exc}"
                )

        if not prepared:
            reason = (
                "engsoccerdata_snapshot_error:"
                + "|".join(snapshot_errors[:3])
                if snapshot_errors
                else "engsoccerdata_snapshot_unavailable"
            )
        ...
        for idx, row in out.loc[mask].iterrows():
            ...
            for observed_at, snapshot, keys in prepared:
                if observed_at < lower_bound_dt or key not in keys:
                    continue
