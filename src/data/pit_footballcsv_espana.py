...
"""Fail-closed PIT evidence from versioned footballcsv Spain snapshots.

The provider is audit-only. It accepts a row only when an immutable Git commit
timestamp is at/after the conservative result-availability lower bound and the
snapshot contains exactly one matching completed-result identity.
"""
...
REPOSITORY = "footballcsv/espana"
COMPETITION = "LL"

SNAPSHOT_PATHS = {
    2019: "2010s/2019-20/es.1.csv",
    2020: "2020s/2020-21/es.1.csv",
}
...
def _headers() -> dict[str, str]:
    headers = {
        ...
        "User-Agent": "SoccerPredictionResearch/footballcsv-LL-PIT",
...
def _request_json(url: str, timeout: int = 30) -> Any:
    ...
    for attempt in range(1, 4):
        try:
            ...
            if attempt >= 3:
                raise RuntimeError(
                    f"github_request_failed:{type(exc).__name__}:{exc}"
                ) from exc
...
def _cache_file(cache_dir: str, prefix: str, *parts: str) -> Path:
    ...
    path = Path(cache_dir) / f"footballcsv-espana-{prefix}-{key}{suffix}"
...
def _commits(path: str, cache_dir: str, timeout: int) -> list[dict[str, Any]]:
    ...
    refresh_hours = float(os.getenv("PIT_FOOTBALLCSV_ESPANA_COMMIT_CACHE_REFRESH_HOURS", "6"))
    ...
    if not isinstance(payload, list):
        raise RuntimeError("invalid_footballcsv_commit_history")
    if payload:
...
def _snapshot(path: str, sha: str, cache_dir: str, timeout: int) -> str:
    ...
    response = requests.get(
        ...
        headers={"User-Agent": "SoccerPredictionResearch/footballcsv-LL-PIT"},
        timeout=timeout,
    )
    response.raise_for_status()
...
def apply_footballcsv_espana_pit(
    ...
    *,
    ...
    for season_start, group in history.loc[mask].groupby("season_start", sort=True):
        ...
        min_bound = min(bound for _, bound in pending.values())
        try:
        ...
        for commit in commits:
            ...
            if dt is not None and sha and dt >= _utc(min_bound):
        ...
        for dt, sha in ordered:
            if not pending:
                break
            eligible = [idx for idx, (_, bound) in pending.items() if dt >= _utc(bound)]
            if not eligible:
                continue
                ...
                verified_rows.append(
                    {
                        ...
                        "pit_evidence_reason": "immutable_footballcsv_espana_snapshot_contains_unique_exact_result_after_result_lower_bound",
                        "pit_evidence_url": f"https://github.com/{REPOSITORY}/blob/{sha}/{path}",
                        "capture_digest": sha,
                    },
                )
                verified_rows[-1]["__index__"] = idx
                del pending[idx]
