...
class FootballDataWaybackAdapter(_FastFootballDataWaybackAdapter):
    ...
    def captures(self, url: str):
        for attempt in range(1, self.cdx_retries + 1):
            ...
            if rows or diag is None or diag.status != "CDX_REQUEST_FAILURE": return rows
            if attempt < self.cdx_retries: time.sleep(self.cdx_retry_backoff * attempt)
    ...
    def _has_exact_capture_identity(final_url, capture_timestamp):
        ...
        if parsed.hostname not in {"web.archive.org", "web.archive.org."}: return False
    ...
    def diagnostic_bulk(self, history: pd.DataFrame) -> pd.DataFrame:
        rows=[]
        if history.empty: return pd.DataFrame(columns=["competition","season_start","url","status","capture_count","error_type","error","cdx_status","failure_stage","failure_reason"])
        work=history.copy()
        if "season_start" not in work.columns and "season" in work.columns: work["season_start"]=work["season"].astype(str).str.extract(r"(\d{4})",expand=False)
        if "season_start" in work.columns: work["season_start"]=pd.to_numeric(work["season_start"],errors="coerce")
        for (competition,start_year),group in work.groupby(["competition","season_start"],dropna=False):
            try:
                ...
                rows.append({"competition":competition,"season_start":start_year,"url":url,"status":diag.status,"capture_count":diag.capture_count,"error_type":diag.error_type,"error":diag.error,"cdx_status":cdx_status,"failure_stage":cdx_status if cdx_status!="CDX_CAPTURE_FOUND" else "","failure_reason":diag.error or ("no archive capture" if cdx_status=="CDX_NO_CAPTURE" else "")})
            except Exception as exc:
                rows.append({"competition":competition,"season_start":start_year,"url":None,"status":"ADAPTER_MAPPING_FAILURE","capture_count":0,"error_type":type(exc).__name__,"error":str(exc),"cdx_status":"NOT_ATTEMPTED","failure_stage":"ADAPTER_MAPPING_FAILURE","failure_reason":str(exc)})
        return pd.DataFrame(rows)


def competition_adapter_matrix() -> pd.DataFrame:
    return pd.DataFrame([{"competition":c,"source":s.get("source"),"source_code":s.get("source_code"),"adapter":s.get("adapter"),"status":"IMPLEMENTED" if s.get("adapter") else "UNVERIFIED"} for c,s in COMPETITION_ADAPTERS.items()])
