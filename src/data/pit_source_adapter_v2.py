...
The adapter distinguishes source capture availability from target-specific PIT
eligibility. It never uses retrieval time as source availability and retains
all CDX timestamps instead of collapsing by digest.
"""
...
def _result_lower_bound(row: pd.Series) -> tuple[datetime | None, str]:
    """Return a conservative earliest timestamp at which the completed result can exist.

    With a precise kickoff, allow 180 minutes for regulation/extra-time completion
    and breaks. With date-only data, require the following UTC day. This is a
    safety lower bound, not a claim about the publisher's actual posting time.
    """
    kickoff = _utc(row.get("kickoff_utc"))
    if kickoff is None:
...
class FootballDataWaybackAdapter:
    ...
    def _prefetch_url(self, url: str, rows: list[pd.Series], workers: int | None = None) -> list[SourceEvidence]:
        ...
        for capture in captures:
            ...
            if ts is None:
                continue
            if any(lb is not None and ts >= lb for lb, _ in bounds):
        ...
        keysets: list[tuple[dict[str, str], SnapshotDiagnostic]] = []
    ...
    def diagnostic_bulk(self, history: pd.DataFrame) -> pd.DataFrame:
        ...
        for (competition, season), group in history.groupby(["competition", "season"], dropna=False):
            ...
            times = [_utc(c.get("timestamp")) for c in captures]
