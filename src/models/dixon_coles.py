...
def dixon_coles_distribution(
    ...
    *,
    ...
    """Return a normalized bivariate Poisson distribution with DC low-score correction."""
    if max_goals < 1:
    ...
    if not math.isfinite(normalizer) or normalizer <= 0:
        raise ValueError("Invalid Dixon-Coles distribution normalizer")
    out = [(h, a, p / normalizer) for h, a, p in cells]
    return sorted(out, key=lambda x: (-float(x[2]), int(x[0]), int(x[1])))


def _safe_rho_bounds(lambdas: list[tuple[float, float]], requested: float = 0.30) -> tuple[float, float]:
    """Keep rho inside a range that preserves positive low-score corrections."""
...
def fit_dixon_coles_model(
    ...
    *,
    ...
    """Fit a DC low-score correction on top of the existing PIT score-rate model.

    The underlying score-rate model is fit only on PIT-verified rows. Rho is then
    optimized using the same historical slice with deterministic recency weights.
    It is a Challenger: callers decide adoption from chronological OOS evidence.
    """
    ...
    base = fit_score_rate_model(history, shrinkage=shrinkage)
    ...
    def objective(rho: float) -> float:
        total = 0.0
        for (lh, la), (h, a), w in zip(lambdas, outcomes, weights):
            ph = math.exp(-lh) * lh**h / math.factorial(h)
            pa = math.exp(-la) * la**a / math.factorial(a)
    ...
    result = minimize_scalar(objective, bounds=(lo, hi), method="bounded", options={"xatol": 0.002})
...
def predict_dixon_coles_distribution(
    ...
    *,
    ...
    return dixon_coles_distribution(lh, la, rho, max_goals=max_goals)
