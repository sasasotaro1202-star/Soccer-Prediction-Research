...
class AdaptiveModelRouter:
    ...
    def fit(
        ...
        *,
        ...
        inverse_global = self._normalize({m: 1.0 / max(s, 1e-6) for m, s in global_scores.items()})
        self._global_weights = self._bounded(inverse_global, self.base_weights)

        if not self.context_columns:
            return self

        for key, group in d.groupby(list(self.context_columns), dropna=False, sort=False):
            if not isinstance(key, tuple):
                key = (key,)
            key = tuple(str(x) for x in key)
            # Count unique prediction timestamps, not model rows, to avoid five-fold
            # inflation when every fixture has one prediction per model.
            ...
            blended = {
                m: alpha * inverse[m] + (1.0 - alpha) * self._global_weights[m]
    ...
    def route(
        ...
        *,
        ...
        missing_models = [m for m in self.models if m not in candidate_probabilities]
        if missing_models:
            raise ValueError(f"Candidate probabilities missing models: {missing_models}")
