...
def _archive_audit_sample(history: pd.DataFrame, out: Path) -> dict:
    ...
    try:
        ...
        rows_per_group = 1
    sample = history.sort_values(["competition", "season_start", "kickoff_utc"], kind="mergesort").groupby(
        ["competition", "season_start"], sort=False, dropna=False, group_keys=False
    ).head(rows_per_group).copy()
    try:
