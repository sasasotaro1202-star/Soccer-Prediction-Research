...
def snapshot_id(df: pd.DataFrame) -> str:
    ...
    for c in stable.columns:
        if pd.api.types.is_datetime64_any_dtype(stable[c]): stable[c] = pd.to_datetime(stable[c], utc=True, errors="coerce").astype("string")
    sort_cols = [c for c in ["competition", "season_start", "kickoff_utc", "home_team", "away_team", "match_id"] if c in stable.columns]
    if sort_cols: stable = stable.sort_values(sort_cols, kind="mergesort")
...
def _model_features(feats: pd.DataFrame) -> list[str]:
    ...
    if not cols: raise ValueError("No numeric model features available")
    return cols


def _archive_audit_sample(history: pd.DataFrame, out: Path) -> dict:
    if os.getenv("PIT_ENABLE_ARCHIVE_AUDIT", "0") != "1": return {"status": "SKIPPED", "reason": "disabled_in_main_research"}
    try: rows_per_group = max(1, int(os.getenv("PIT_REPLAY_ROWS_PER_GROUP", "1")))
    except ValueError: rows_per_group = 1
...
def run(out_dir: str = "artifacts") -> dict:
    ...
    _write_source_registry(out); competition_adapter_matrix().to_csv(out / "pit_competition_adapter_matrix.csv", index=False)
    ...
    if history.empty:
        ...
        (out / "run_status.json").write_text(json.dumps(report, indent=2, ensure_ascii=False)); return report
    ...
    if pit_verified == 0:
        ...
        (out / "run_status.json").write_text(json.dumps(report, indent=2, ensure_ascii=False, default=str)); return report

    wf, selections = run_walk_forward(feats, _model_features(feats))
    wf.to_csv(out / "oos_metrics.csv", index=False); selections.to_csv(out / "model_selection.csv", index=False)
    locked = wf.tail(1).copy(); locked.to_csv(out / "locked_oos_metrics.csv", index=False)
    ...
    candidate = locked[["oos_start", "oos_end", "logloss", "accuracy", "brier", "ece", "n"]].copy()
    adoption = adoption_decision(baseline, candidate, min_accuracy=TARGET_ACCURACY)
    ...
    report = {"status": "OK", "snapshot_id": snapshot_id(history), "oos": wf.mean(numeric_only=True).to_dict(), "coverage": coverage.to_dict(orient="records"), "archive_audit": archive_audit, "pit_policy": "result_plus_24h", "pit_verified_rows": pit_verified, "pit_total_rows": pit_total, "pit_verified_rate": float(pit_verified / pit_total) if pit_total else 0.0, "accuracy_target": {"target_accuracy": TARGET_ACCURACY, "locked_oos_accuracy": float(locked["accuracy"].iloc[0]), "weighted_oos_accuracy": oos_acc, "target_met": bool(oos_acc >= TARGET_ACCURACY) if oos_n else False, "target_gap": float(oos_acc - TARGET_ACCURACY) if oos_n else float("nan"), "oos_sample_size": oos_n}, "adoption": adoption, "production_model": "ensemble" if adoption.get("status") == "ADOPT" else "baseline_logistic", "ai_research": weakness_advice(wf.mean(numeric_only=True).to_dict())}
    (out / "run_status.json").write_text(json.dumps(report, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
    return report


if __name__ == "__main__": print(json.dumps(run(), indent=2, ensure_ascii=False, default=str))
