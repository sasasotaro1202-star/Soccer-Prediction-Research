...
def test_fit_rejects_future_outcome_evidence():
    d = _oos()
    d.loc[d.index[-1], "outcome_available_at_utc"] = pd.Timestamp("2027-01-01", tz="UTC")
    router = AdaptiveModelRouter(config=RouterConfig(min_context_rows=10))
    router.fit(d, as_of="2025-08-30T00:00:00Z")
    assert all(v >= 0.0 for v in router._global_weights.values())
...
def test_router_is_deterministic_and_valid():
    router = AdaptiveModelRouter(config=RouterConfig(min_context_rows=10))
    d = _oos()
    router.fit(d)
...
def test_context_missing_columns_are_supported():
