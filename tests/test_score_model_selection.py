...
def _frame(recency_better=True):
    primary = [1.00, 1.02, 0.98]
    recency = [0.98, 1.00, 0.97] if recency_better else [1.03, 1.04, 1.00]
    dc = [1.01, 1.01, 0.99]
    return pd.DataFrame(
        {
            "n": [100, 100, 100],
            "score_logloss": primary,
            "over_2_5_logloss": [0.70, 0.72, 0.71],
            "over_2_5_brier": [0.20, 0.21, 0.19],
            "btts_logloss": [0.68, 0.69, 0.67],
            "btts_brier": [0.21, 0.22, 0.20],
            "recency_score_logloss": recency,
...
def test_selector_adopts_recency_only_from_development_oos():
    result = select_score_model(_frame(True), min_blocks=2)
    assert result["selected_method"] == "recency"
    assert result["status"] == "ADOPT_CANDIDATE"
    assert result["selection_rule"]["locked_oos_inspected"] is False


def test_selector_keeps_primary_when_challenger_is_worse():
    result = select_score_model(_frame(False), min_blocks=2)
    assert result["selected_method"] == "primary"
    assert result["status"] == "KEEP_PRIMARY"


def test_unavailable_challenger_never_blocks_primary_selection():
    ...
    result = select_score_model(frame, min_blocks=2)
    assert result["selected_method"] == "recency"
